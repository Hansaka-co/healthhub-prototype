const TYPES=['Dental check-up','General consultation','Cardiology review','Eye test','Physiotherapy','Vaccination'];
  ...
  function go(id,back=false){
    ...
    $('nav').style.display=tabScreens.includes(id)?'flex':'none';
  ...
  function renderReminderRows(){
    $('rem-rows').innerHTML=OFFSETS.map(o=>`
      <div class="trow" onclick="toggleRem('${o.key}')">
        <div class="tile teal" style="width:40px;height:40px;background:${state.rem[o.key]?'var(--brand-tint)':'#F2F4F3'}">
          <svg width="20" height="20"><path d="M4 13 q0 -10 6 -10 q6 0 6 10 l2 3 l-16 0 z" fill="none" stroke="${state.rem[o.key]?'#0E5A54':'#9AA6A2'}" stroke-width="1.6"/><path d="M8 16 q2 2.5 4 0" fill="none" stroke="${state.rem[o.key]?'#0E5A54':'#9AA6A2'}" stroke-width="1.6"/></svg></div>
  ...
  function apptCard(o){
    ...
    return `<div class="card"><div class="crow"><div class="tile teal">${o.icon==='tooth'?tooth:heart}</div>
      <div style="flex:1"><div class="c-title">${o.title}</div><div class="c-sub">${o.sub}</div></div></div>
